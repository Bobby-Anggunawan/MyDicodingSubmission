...
object SetUpRecyclerView {
    ...
    class myViewHolder<T>(val ItemLayout: View, val Data: ArrayList<T>, val Items: buildItemStorage, val onItemClick: ((T) -> Unit)?): RecyclerView.ViewHolder(ItemLayout){
    ...
    class myAdapter<T>(val Data: ArrayList<T>, val itemLayout: Int, val Items: buildItemStorage, val onItemClick: ((T) -> Unit)?): RecyclerView.Adapter<myViewHolder<T>>(){
        ...
        override fun onBindViewHolder(holder: myViewHolder<T>, position: Int) {
            ...
            holder.imageList.forEach {
                val value = readInstanceProperty<String>(myItem as Any, it.propertyName)
                Glide.with(holder.itemView.context).load("https://www.themoviedb.org/t/p/w220_and_h330_face"+value).into(it.myImageView)
            }
