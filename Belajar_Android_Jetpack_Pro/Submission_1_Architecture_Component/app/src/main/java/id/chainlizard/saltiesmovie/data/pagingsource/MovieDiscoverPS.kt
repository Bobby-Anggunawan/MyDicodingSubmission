...
class MovieDiscoverPS(private var repository: MyRepository, private var type: MyObj.pageType) :
    PagingSource<Int, MovieDiscoverMod.MoviePage_List>() {
    ...
    override suspend fun load(params: LoadParams<Int>): LoadResult<Int, MovieDiscoverMod.MoviePage_List> {
        return try {
            ...
            if (type == MyObj.pageType.Discover) {
                val response =
                    withContext(Dispatchers.Default) { repository.getMovieDiscover(nextPageNumber) }
                LoadResult.Page(
                ...
                val response =
                    withContext(Dispatchers.Default) { repository.getMovieWLAsc(nextPageNumber) }
                LoadResult.Page(
