...
class TVDiscoverPS(private val repository: MyRepository, private val type: MyObj.pageType) : PagingSource<Int, TVDiscoverMod.TVPage_List>(){
    override suspend fun load(params: LoadParams<Int>): LoadResult<Int, TVDiscoverMod.TVPage_List> {
        return try {
            ...
            if(type == MyObj.pageType.Discover){
                ...
                val response = withContext(Dispatchers.Default) { repository.getTVWLAsc(nextPageNumber)}
                LoadResult.Page(
